...
class RealNVP(bg.SequentialFlow):
    ...
    def _dense_net(self, dim1, dim2):
        return bg.DenseNet([dim1, *self.hidden, dim2], activation=torch.nn.ReLU())
...
generator = bg.BoltzmannGenerator(flow=flow, prior=prior, target=target_energy)
...
nll_trainer = bg.KLTrainer(generator, optim=nll_optimizer, train_energy=False)
...
if main:
    nll_trainer.train(
        n_iter=20000, data=training_data, batchsize=128, n_print=1000, w_energy=0.0
    )
...
mixed_trainer = bg.KLTrainer(generator, optim=mixed_optimizer, train_energy=True)
