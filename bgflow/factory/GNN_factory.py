...
def make_allegro_config_dict(**kwargs):
    ...
    base_dict = {
        "one_hot": (
            nequip.nn.embedding._one_hot.OneHotAtomEncoding,
            {"irreps_in": None, "set_features": True, "num_types": num_types},
        ),
        "radial_basis": (
            nequip.nn.embedding._edge.RadialBasisEdgeEncoding,
            {
        ...
        "spharm": (
            nequip.nn.embedding._edge.SphericalHarmonicEdgeAttrs,
            {
    ...
    atomwise_dict = {
        "atomwise_gather": (
            allegro.nn._edgewise.EdgewiseReduce,
            {
                "field": "edge_features",
        ...
        "atomwise_linear": (
            nequip.nn._atomwise.AtomwiseLinear,
            {
    ...
    bondwise_dict = {
        "bondwise_linear": (
            nequip.nn._atomwise.AtomwiseLinear,  ## it is still edgewiese
            {
                "field": "edge_features",
                "out_field": "outputs",
                "irreps_out": f"{GNN_feature_dim}x0e",
...
def make_nequip_config_dict(**kwargs):  ###rename to make_nequip_config_dict
    ...
    base_dict = {
        ...
        "chemical_embedding": (
            nequip.nn._atomwise.AtomwiseLinear,
            {"field": "node_features", "out_field": None, "irreps_out": "32x0e"},
        ),
    }
    conv_dict = {}
    ...
    for i in range(num_interaction_blocks):
        conv_dict_i = {
            f"convnet_{i}": (
                nequip.nn._convnetlayer.ConvNetLayer,
                {
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(nequip_GNN, torch.nn.Module):
            self.GNN = (
                nequip_GNN  # register the GNN that is to be used in the conditioner
            )
...
class WrapDistancesGNN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        binned_enveloped_distances = (u.unsqueeze(-1) * binned_distances).view(
            x.shape[0], -1
        )
