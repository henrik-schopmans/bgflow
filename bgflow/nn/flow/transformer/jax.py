...
def smooth_ramp(x, logalpha, power=1, eps=1e-9):
    ...
    normalizer = jnp.exp(-alpha * 1.0)
    return jnp.where(
        x > eps, jnp.exp(-alpha * jnp.power(z, -power)) / normalizer, jnp.zeros_like(z)
    )
...
def affine_sigmoid(sigmoid, eps=1e-8):
    ...
    def _affine_sigmoid(x, shift, slope, mix, *params):
        slope = jnp.exp(slope)
        mix = jax.nn.sigmoid(mix) * (1.0 - eps) + eps
        return mix * sigmoid(slope * (x - shift), *params) + (1.0 - mix) * x
...
def mixture(bijector):
    ...
    def _mixture_bijector(x, weights, *params):
        components = jax.vmap(functools.partial(bijector, x), in_axes=-1, out_axes=-1)(
            *params
        )
