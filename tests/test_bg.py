...
def test_bg_metrics(ctx, is_flow_exact):
    ...
    target = TorchDistribution(
        torch.distributions.Independent(
            torch.distributions.Normal(
                torch.tensor((1.0,), **ctx), torch.tensor((2.0,), **ctx)
            ),
            1,
        )
    )
    ...
    class Shift(Flow):
        ...
        def _forward(self, x, **kwargs):
            return (
                self.logscale.exp() * x + self.shift,
                self.logscale * torch.ones_like(x).sum(dim=-1, keepdim=True),
            )

        def _inverse(self, x, **kwargs):
            return (
                x - self.shift
            ) / self.logscale.exp(), -self.logscale * torch.ones_like(x).sum(
                dim=-1, keepdim=True
            )

    shift = 1.0 if is_flow_exact else 5.0
    scale = 2.0 if is_flow_exact else 0.01
    ...
    with torch.no_grad():
        ...
        assert is_flow_exact == torch.allclose(
            prior.energy(z) + dlogp, target.energy(x)
        )
        assert is_flow_exact == torch.allclose(gen.energy(x), target.energy(x))

        # weights
        logw = log_weights(x, prior=prior, flow=flow, target=target)
        assert is_flow_exact == torch.allclose(
            logw, torch.log(1 / len(logw) * torch.ones_like(logw)), atol=1e-4
        )
        assert torch.allclose(
            logw, log_weights_given_latent(x, z, dlogp, prior, target), atol=1e-4
        )
        assert torch.allclose(
            logw,
            gen.log_weights(x),
        ...
        assert torch.allclose(
            logw, gen.log_weights_given_latent(x, z, dlogp), atol=1e-4
        )
        effsize = effective_sample_size(logw)
        assert is_flow_exact == torch.allclose(
            effsize, torch.tensor(10.0, **ctx), atol=1e-4
        )
        seff = sampling_efficiency(logw)
        assert is_flow_exact == torch.allclose(seff, torch.ones_like(seff), atol=1e-4)
    ...
    assert torch.allclose(nll, gen.energy(x))
    nll.mean().backward()
    # -- optimal gradients
    assert is_flow_exact == torch.allclose(
        flow.shift.grad, torch.zeros_like(flow.shift.grad), atol=1e-1
    )
    assert is_flow_exact == torch.allclose(
        flow.logscale.grad, torch.zeros_like(flow.logscale.grad), atol=1e-1
    )
...
def test_bg_basic_multiple(device, dtype):
    ...
    prior = bg.ProductDistribution(
        [bg.NormalDistribution(dim // 2, mean), bg.NormalDistribution(dim // 2, mean)]
    )
    # RealNVP
    flow = bg.SequentialFlow(
        [
    ...
    *z, dlogp = flow.forward(*samples, inverse=True)
    ...
    sampler = bg.ProductSampler(
        [bg.DataSetSampler(samples[0]), bg.DataSetSampler(samples[1])]
