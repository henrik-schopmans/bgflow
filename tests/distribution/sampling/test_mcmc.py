...
@pytest.mark.parametrize(
    ...
    [
        ...
        LatentProposal(
            flow=BentIdentity(), base_proposal=GaussianProposal(noise_std=0.4)
        ),
    ],
)
...
def test_mcmc(ctx, proposal, temperatures):
    ...
    mcmc = IterativeSampler(
        ...
        sampler_steps=[
            MCMCStep(
                target, proposal=proposal.to(**ctx), target_temperatures=temperatures
            )
        ],
        stride=2,
        n_burnin=100,
    ...
    assert torch.allclose(
        samples.mean(dim=(0, 1, 3)), torch.tensor([3.0] * 3, **ctx), atol=0.1
    )
...
def test_old_vs_new_mcmc(ctx):
    ...
    new_mc = GaussianMCMCSampler(
        energy, x0, stride=10, n_burnin=10, noise_std=0.3, box_constraint=constraint
    )
    ...
    assert old_samples.mean().item() == pytest.approx(
        new_samples.mean().item(), abs=1e-2
    )
