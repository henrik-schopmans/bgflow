...
class AddOne(SamplerStep):
    def _step(self, state: AbstractSamplerState):
        ...
        samples = tuple(x + 1.0 for x in statedict["samples"])
...
def test_iterative_sampler(ctx):
    ...
    assert torch.allclose(
        sampler.state.samples[0], 10 * torch.ones_like(sampler.state.samples[0])
    )

    # test sampling
    samples = sampler.sample(2)
    assert torch.allclose(samples, torch.tensor([[11.0, 11.0], [12.0, 12.0]], **ctx))

    # test stride
    sampler.stride = 5
    samples = sampler.sample(2)
    assert sampler.i == 14
    assert torch.allclose(samples, torch.tensor([[17.0, 17.0], [22.0, 22.0]], **ctx))

    # test iteration
    sampler.max_iterations = 15
    for batch in sampler:  # only called once
        assert torch.allclose(batch.samples[0], torch.tensor([[27.0, 27.0]], **ctx))
