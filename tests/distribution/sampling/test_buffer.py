...
def test_replay_buffer_sampling(use_product, with_reporter, ctx, tmpdir):
    ...
    if with_reporter:
        reporter = ReplayBufferHDF5Reporter(
            tmpdir / "test.h5", "w", write_buffer_interval=80
        )
    else:
        reporter = None
...
def test_hdf5_file(tmpdir, ctx, use_product):
    ...
    hdf5.write_accepted_samples(
        *samples, energies=energies, indices=np.arange(10), step=0, forced_update=True
    )
    hdf5.close()
    ...
    assert np.allclose(
        buffer_energies.astype(float), energies2.detach().cpu().numpy().astype(float)
    )
