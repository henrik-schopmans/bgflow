...
def test_distribution_energy(dim, device, dtype):
    ...
    assert torch.allclose(
        normal_trch.energy(samples), normal_bgtrch.energy(samples), rtol=2e-2, atol=1e-2
    )
...
def test_distribution_samples(dim, sample_shape, device, dtype):
    ...
    target_shape = (
        torch.Size([sample_shape]) if isinstance(sample_shape, int) else sample_shape
    )
...
def test_sample_uniform_with_temperature(ctx):
    uniform = UniformDistribution(
        low=torch.zeros(100, **ctx), high=torch.ones(100, **ctx)
    )
    assert uniform.sample(20).mean().item() == pytest.approx(0.5, abs=0.05)
    assert uniform.sample(20, temperature=100.0).mean().item() == pytest.approx(
        0.5, abs=0.05
    )
...
def test_sample_product_with_temperature(ctx):
    ...
    x1, y1 = product.sample(20, temperature=1.0)
    x2, y2 = product.sample(20, temperature=100.0)
