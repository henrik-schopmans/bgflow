...
pytestmark = pytest.mark.skipif(
    not ase_and_xtb_imported, reason="Tests require ASE and XTB"
)
...
def test_ase_energy(ctx):
    ...
    water = molecule("H2O")
    water.calc = XTB()
    target = ASEEnergy(ASEBridge(water, 300.0))
...
def test_ase_vs_xtb(ctx):
    ...
    water = molecule("H2O")
    water.calc = XTB()
    target1 = ASEEnergy(ASEBridge(water, 300.0))
    target2 = XTBEnergy(XTBBridge(water.numbers, 300.0))
