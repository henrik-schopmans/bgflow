...
def test_linlogcut(ctx):
    ...
    x = torch.tensor(
        [
            [[0.0, 0.0], [0.0, 0.0]],  # > max energy
            [[0.0, 0.0], [0.0, 0.3]],  # < max_energy, > high_energy
            [[0.0, 0.0], [0.0, 1.0]],  # < high_energy
        ],
        **ctx
    )
    raw = lj.energy(x)[:, 0]
...
def openmm_example(grad_clipping, ctx):
    ...
    bridge = OpenMMBridge(
        system, openmm.LangevinIntegrator(300.0, 0.1, 0.001), n_workers=1
    )
...
def test_openmm_clip_by_value(ctx):
    ...
    expected = -torch.as_tensor(
        [[-1908.0890, -3000.0, -3000.0, 1908.0890, 3000.0, 3000.0]], **ctx
    )
...
def test_openmm_clip_by_atom(ctx):
    ...
    assert norm_ratio == pytest.approx(3000.0)
    assert torch.allclose(grad[..., :3] / 3000.0, -grad[..., 3:], atol=1e-6)
...
def test_openmm_clip_by_batch(ctx):
    ...
    assert torch.linalg.norm(grad).item() == pytest.approx(1.0)


def test_openmm_clip_no_grad(ctx):
    energy = GradientClippedEnergy(
        energy=DoubleWellEnergy(2), gradient_clipping=ClipGradient(clip=1.0, norm_dim=1)
    )
