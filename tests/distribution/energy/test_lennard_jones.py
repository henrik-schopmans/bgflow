...
def test_lennard_jones_energy_torch():
    ...
    energy_min = lennard_jones_energy_torch(torch.tensor(5.0), eps=3, rm=5)
    energy_zero = lennard_jones_energy_torch(torch.tensor(2 ** (-1 / 6)), eps=3, rm=1)
    assert torch.allclose(energy_large, torch.tensor(0.0))
    assert torch.allclose(energy_min, torch.tensor(-3.0))
    assert torch.allclose(energy_zero, torch.tensor(0.0), atol=1e-5)
...
def test_lennard_jones_potential(oscillator, two_event_dims):
    ...
    batch_shape = (5, 7)
    data3d = torch.tensor([[[[-1.0, 0, 0], [1, 0, 0]]]]).repeat(*batch_shape, 1, 1)
    ...
    h = np.sqrt(0.75)
    data2d = torch.tensor(
        [[[0, 2 / 3 * h], [0.5, -1 / 3 * h], [-0.5, -1 / 3 * h]]], dtype=torch.float
    )
    if not two_event_dims:
