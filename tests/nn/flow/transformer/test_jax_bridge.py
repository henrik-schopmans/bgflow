...
@pytest.mark.skipif(
    jax is None or jnp is None, reason="skipping test due missing jax installation"
)
def test_simple_mixture_transformer(ctx):
    with double_raises(ctx["dtype"]):
        bijector = chain(
            wrap_around, mixture, affine_sigmoid, ramp_to_sigmoid, smooth_ramp
        )
        ...
        weights = torch.from_numpy(
            np.random.normal(size=(batch, features, num_components))
        ).to(**ctx)
        ...
        assert torch.allclose(ldjy + ldjz, torch.zeros_like(ldjz), rtol=1e-4, atol=1e-4)
...
def get_grads(bijector, x, *cond):
    ...
    return (
        vjp_fun((jnp.ones_like(out_y), jnp.zeros_like(out_ldj))),
...
def test_approx_inv_gradients():
    ...
    for fwd, inv in zip(bijectors, inverses):
        ...
        approx_inv = invert_bijector(
            fwd, functools.partial(bisect, left_bound=-10, right_bound=10.0, eps=1e-20)
        )
        ...
        a = jnp.array(np.random.uniform(low=0.1, high=10.0, size=(1000)))
        b = jnp.array(np.random.uniform(low=0.0, high=1.0, size=(1000)))
        ...
        for label, err_fn in zip(
            ["abs err", "rel err"], [abs_err, functools.partial(rel_err, eps=threshold)]
        ):
            err_val = np.max(
...
def test_bgflow_interface(ctx):
    with double_raises(ctx["dtype"]):
        ...
        def compute_params(x, y_shape):
            ...
            return tuple(
                p.view(*p.shape[:-1], y_shape[-1], num_mixtures) for p in params
            )
        ...
        transformer = JaxTransformer(
            chain(
                functools.partial(
                    wrap_around,
                    sheaves=jnp.linspace(-10, 10, 21),
                    weights=jnp.exp(-jnp.linspace(-10, 10, 21)),
                ),
                mixture,
                affine_sigmoid,
                jax.nn.tanh,
        ...
        assert torch.allclose(ldj1, -ldj2, atol=1e-5, rtol=1e-3), (
            (ldj1 + ldj2).abs().max()
        )
