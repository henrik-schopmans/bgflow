...
@pytest.mark.parametrize(
    "model", [InvertiblePPPP(3), SequentialFlow([InvertiblePPPP(3), InvertiblePPPP(3)])]
)
def test_scheduler(model):
    """API test for a full optimization workflow."""
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-1)
    scheduler = PPPPScheduler(
        model, optimizer, n_force_merge=5, n_correct=5, n_recompute_det=5
    )
    torch.manual_seed(0)
    a = 1e-6 * torch.eye(3)
