...
def test_split_flow(ctx):
    tensor = torch.arange(0, 12.0, **ctx).reshape(3, 4)
    ...
    *result, dlogp = split.forward(tensor)
    ...
    assert dlogp.shape == (
        1,
        4,
    )  #  <- this does not make sense yet until we allow event shapes


def test_split_with_indices(ctx):
    tensor = torch.arange(0, 12.0, **ctx).reshape(1, 3, 4)
    indices = [[0, 2], [1]]
...
def test_split_with_indices_failure(ctx, indices):
    ...
    with pytest.raises(Exception):
        tensors = (
            torch.arange(0, 8.0, **ctx).reshape(1, 2, 4),
            torch.arange(8, 16.0, **ctx).reshape(1, 2, 4),
        )
...
def test_coupling_multiple_conditioned(device, dtype):
    ...
    assert torch.allclose(y1, x1 + 2.0)
...
def test_coupling_multiple_transformed(device, dtype):
    ...
    coupling = CouplingFlow(
        transformer, transformed_indices=(0, 2), cond_indices=(3, 1)
    )
