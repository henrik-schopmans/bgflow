from bgflow.nn.flow.stochastic.snf_openmm import (
...
pytestmark = [
    ...
    pytest.mark.filterwarnings(
        "ignore:The current implementation of the BrownianPathProbabilityIntegrator"
    ),
]
...
def make_harmonic_system(n_particles, force_constant):
    ...
    system.setDefaultPeriodicBoxVectors(
        mm.Vec3(1, 0, 0), mm.Vec3(0, 1, 0), mm.Vec3(0, 0, 1)
    )
...
def test_path_probability(IntegratorClass):
    ...
    assert (
        abs(ratios["base"]) < 0.5
    )  # correct distribution should have a log path density ratio of approximately 0

    # check if we can bind the reverse integrator to a context and use it
    mm.Context(
        system,
        integ.get_reverse_integrator(),
...
def test_flow_bridge(temperature, n_workers):
    ...
    bridge = OpenMMBridge(
        ala2.system, integrator, n_workers=n_workers, n_simulation_steps=4
    )
    snf = OpenMMStochasticFlow(bridge)

    batch_size = 2
    x = torch.tensor(
        np.array([ala2.positions.value_in_unit(unit.nanometer)] * batch_size)
    ).view(batch_size, len(ala2.positions) * 3)
    y, dlogP = snf._forward(x)
    assert not torch.all(
        x.isclose(y, atol=1e-3)
    )  # assert that output differs from input
    assert torch.all(
        x.isclose(y, atol=0.5)
    )  # assert that output does not differ too much
    if temperature < 300:
        assert (
            dlogP[0].item() < 0.0
        )  # should be a space-contracting update since temperature is low
    else:
        assert (
            dlogP[0].item() > 0.0
        )  # should be a space-expanding update since temperature is high
...
def test_temperature(IntegratorClass):
    ...
    for integ, name in [(integrator, "here"), (reference_integrator, "ref")]:
        ...
        for _ in range(n_steps):
            ...
            pe[name] += state.getPotentialEnergy().value_in_unit(
                unit.kilojoules_per_mole
            ) / (n_particles * n_steps)
            ke[name] += state.getPotentialEnergy().value_in_unit(
                unit.kilojoules_per_mole
            ) / (n_particles * n_steps)
    ...
    assert np.isclose(
        x_moments["here"], x_moments["ref"], atol=0.1
    ).all()  # distribution of positions
    assert np.isclose(
        v_moments["here"][1], v_moments["ref"][1], atol=0.1
    ).all()  # average velocities ~ 0
    assert np.isclose(
        v_moments["here"][2], v_moments["ref"][2], rtol=0.2
    ).all()  # average KE/particle
