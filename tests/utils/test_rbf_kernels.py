...
def test_kernelize_with_rbf():
    ...
    distances = torch.tensor([[[[0.0], [5]], [[0], [5]], [[5], [5]]]])
    mus = torch.tensor([[[[0.0, 5]]]])
    gammas = torch.tensor([[[[1.0, 0.5]]]])

    distances2 = torch.tensor([[[[1.0]]]], requires_grad=True)
    mus2 = torch.tensor([[[[1.0, 3]]]])
    gammas2 = torch.tensor([[[[1.0, 0.5]]]])
    ...
    assert torch.allclose(
        rbfs,
        torch.tensor([[[[1.0, 0], [0, 1]], [[1.0, 0], [0, 1]], [[0, 1], [0, 1]]]]),
        atol=1e-5,
    )
...
def test_rbf_kernels():
    ...
    rbfs, derivatives_rbfs = rbf_kernels(
        distances, mus, neg_log_gammas, derivative=True
    )
    assert torch.allclose(
        rbfs,
        torch.tensor([[[[1.0, 0], [0, 1]], [[1.0, 0], [0, 1]], [[0, 1], [0, 1]]]]),
    ...
    rbfs2, derivatives_rbfs2 = rbf_kernels(
        distances2, mus2, neg_log_gammas2, derivative=True
    )
    assert torch.allclose(rbfs2, torch.cat([r1, r2], dim=-1), atol=1e-5)
    ...
    assert torch.allclose(
        derivatives_rbfs2, torch.cat([dr1[0], dr2[0]], dim=-1), atol=1e-5
    )
