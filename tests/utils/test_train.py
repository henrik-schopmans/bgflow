...
def test_clip_by_val(ctx):
    grad_clipping = ClipGradient(clip=3.0, norm_dim=1)
    assert torch.allclose(
        torch_example(grad_clipping, ctx),
        torch.tensor([[0.0, 1.0, 2.0], [3.0, 3.0, 3.0]], **ctx),
    )


def test_clip_by_atom(ctx):
    grad_clipping = ClipGradient(clip=3.0, norm_dim=3)
    norm2 = torch.linalg.norm(torch.arange(3, 6, **ctx)).item()
    assert torch.allclose(
        torch_example(grad_clipping, ctx),
        torch.tensor(
            [[0.0, 1.0, 2.0], [3 / norm2 * 3, 4 / norm2 * 3, 5 / norm2 * 3]], **ctx
        ),
    )
...
def test_clip_by_batch(ctx):
    ...
    assert torch.allclose(
        torch_example(grad_clipping, ctx),
        (torch.arange(6, **ctx) / norm2 * 3.0).reshape(2, 3),
    )
