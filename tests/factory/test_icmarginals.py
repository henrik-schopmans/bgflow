...
def test_icmarginals_inform_api(tmpdir, ctx, with_data):
    ...
    dataset = bgmol.datasets.Ala2Implicit1000Test(root=tmpdir, download=True, read=True)
    ...
    current_dims[BONDS] = (
        coordinate_transform.dim_bonds - dataset.system.system.getNumConstraints(),
    )
    current_dims[ANGLES] = (coordinate_transform.dim_angles,)
    marginals = InternalCoordinateMarginals(current_dims, ctx)
    if with_data:
        constrained_indices, _ = bgmol.bond_constraints(
            dataset.system.system, coordinate_transform
        )
        marginals.inform_with_data(
            torch.tensor(dataset.xyz, **ctx),
