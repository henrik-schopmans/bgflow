...
def test_transformers(crd_trafo, transformer_type):
    ...
    transformer = make_transformer(
        transformer_type, (BONDS,), shape_info, conditioners=conditioners
    )
    out = transformer.forward(
        torch.zeros(2, shape_info[FIXED][0]), torch.zeros(2, shape_info[BONDS][0])
    )
...
def test_circular_affine(crd_trafo):
    ...
    with pytest.raises(ValueError):
        conditioners = make_conditioners(
            bgflow.AffineTransformer, (TORSIONS,), (FIXED,), shape_info=shape_info
        )
        make_transformer(
            bgflow.AffineTransformer, (TORSIONS,), shape_info, conditioners=conditioners
        )
    ...
    out = transformer.forward(
        torch.zeros(2, shape_info[FIXED][0]), torch.zeros(2, shape_info[TORSIONS][0])
    )
