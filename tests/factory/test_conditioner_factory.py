...
pytestmark = pytest.mark.skipif(
    not GNN_LIBRARIES_IMPORTED, reason="tests require nequip library"
)
...
def test_conditioner_factory_input_dim(transformer_type, crd_trafo):
    ...
    conditioners = make_conditioners(
        transformer_type, (BONDS,), (FIXED,), shape_info, hidden=(128, 128)
    )
    ...
    conditioners = make_conditioners(
        transformer_type, (BONDS,), (ANGLES, TORSIONS), shape_info, hidden=(128, 128)
    )
    for conditioner in conditioners.values():
        assert conditioner.net._layers[0].weight.shape == (
            128,
            shape_info[ANGLES][0] + 2 * shape_info[TORSIONS][0],
        )
    ...
    for conditioner in conditioners.values():
        ...
        x = torch.cat(
            [torch.zeros(shape_info[ANGLES][0]), torch.ones(shape_info[TORSIONS][0])]
        )
        high = conditioner(x)
...
def test_GNN(
    ala2, crd_trafo_unwhitened, conditioner, use_checkpointing, attention_level
):
    ...
    if conditioner == "allegro":
        ...
        from bgflow.factory.GNN_factory import (
            allegro_hparams as hparams,
            make_allegro_config_dict as make_config_dict,
        )

    if conditioner == "nequip":
        from bgflow.factory.GNN_factory import (
            nequip_hparams as hparams,
    ...
    if conditioner in ["allegro", "nequip"]:
        ...
        fixed = crd_trafo_unwhitened.forward(
            torch.Tensor(ala2.xyz.reshape(ala2.xyz.shape[0], -1))
        )[3]
        RBF_distances = distances_net(fixed.detach()).flatten().cpu()
        RBF_distances = RBF_distances[RBF_distances <= hparams["r_max"]]

        avg_num_neighbors = (
            (len(RBF_distances) / (ala2.xyz.shape[0] * fixed.shape[1] / 3))
        ) * 2
        ...
        if conditioner == "nequip":
            for i in range(hparams["num_interaction_blocks"]):
                layers[f"convnet_{i}"][1]["convolution_kwargs"][
                    "avg_num_neighbors"
                ] = avg_num_neighbors
    ...
    if conditioner in ["allegro", "nequip"]:
        ...
        kwargs = {
            ...
            "GNN_kwargs": {"num_basis": num_basis, "r_max": 15, "env_p": 48},
            "use_checkpointing": use_checkpointing,  # False,
            "GNN_output_dim": (
                (shape_info[FIXED][0] // 3) ** 2 - shape_info[FIXED][0] // 3
            )
            // 2
            * num_basis,
            "attention_units": (
