import numpy as np
from bgflow.factory.tensor_info import (
    ShapeDictionary,
    TensorInfo,
    BONDS,
    ANGLES,
...
def test_shape_info(crd_trafo):
    ...
    assert (
        ...
        == np.arange(
            shape_info[FIXED][0], shape_info[FIXED][0] + shape_info[TORSIONS][0]
        )
    ).all()
    assert (
        shape_info.circular_indices()
        == np.arange(
            shape_info[BONDS][0] + shape_info[ANGLES][0],
            shape_info[BONDS][0] + shape_info[ANGLES][0] + shape_info[TORSIONS][0],
        )
    ).all()
    assert (
        shape_info.dim_all([BONDS, TORSIONS])
        == shape_info[BONDS][0] + shape_info[TORSIONS][0]
    )
    ...
    assert (
        shape_info.dim_noncircular([ANGLES, BONDS])
        == shape_info[ANGLES][0] + shape_info[BONDS][0]
    )
    ...
    assert (
        shape_info.dim_noncartesian([ANGLES, BONDS])
        == shape_info[ANGLES][0] + shape_info[BONDS][0]
    )
